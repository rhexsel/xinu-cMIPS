...
pid32 nextpid;	/* position in table to try or	*/
                /*  one beyond end of table, in initialize.c, to global RH */
	...
	)
{
	...
	// prptr->prstkptr = (char *)saddr; // this is repeated below RH
	...
	*--saddr = (uint32)prptr->prstkbase - prptr->prstklen + sizeof(int);
...
local	pid32	newpid(void)
{
	...
	for (i = 0; i < NPROC; i++) {
		nextpid = nextpid % NPROC;	/* wrap around to beginning */
